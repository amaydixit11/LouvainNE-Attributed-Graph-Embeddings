...
BLOCK_SIZE = 1024
EXACT_TOPK_MAX_NODES = 25000
EDGE_SIMILARITY_BATCH_SIZE = 200000
...
def build_structure_reweighted_edges(
    edge_index: torch.Tensor,
    normalized_features: torch.Tensor,
    min_similarity: float,
    overlap_scale: float = 1.0,
    batch_size: int = EDGE_SIMILARITY_BATCH_SIZE,
) -> Tuple[Dict[Tuple[int, int], float], Dict[Tuple[int, int], float]]:
    """Reweight observed structure edges using attribute similarity.

    This is linear in the number of observed edges and is therefore suitable for
    large OGB graphs where global all-pairs top-k similarity is too expensive.
    """
    undirected_edges = unique_undirected_edges(edge_index)
    fused_edges: Dict[Tuple[int, int], float] = {}
    predicted_edges: Dict[Tuple[int, int], float] = {}

    if not undirected_edges:
        return fused_edges, predicted_edges

    for start in range(0, len(undirected_edges), batch_size):
        batch = undirected_edges[start:start + batch_size]
        src = torch.tensor([u for u, _ in batch], dtype=torch.long)
        dst = torch.tensor([v for _, v in batch], dtype=torch.long)
        sims = (normalized_features[src] * normalized_features[dst]).sum(dim=1)

        for (u, v), sim in zip(batch, sims.tolist()):
            if sim >= min_similarity:
                score = float(sim)
                fused_edges[(u, v)] = 1.0 + overlap_scale * score
                predicted_edges[(u, v)] = score
            else:
                fused_edges[(u, v)] = 1.0
...
def evaluate_ogb_with_timing(
    ...
    if structure_edges is not None:
        lp_seed = lp_seeds[0]
        _, val_pos, test_pos, train_edge_index, _, test_neg = create_link_prediction_split(
            data.edge_index, seed=lp_seed, val_ratio=0.02, test_ratio=0.05
        )
        train_predicted = {}
        train_fused = structure_edges
        ...
        lp_feature_embedding = None
        if feature_matrix is not None and lp_feature_dim > 0:
            lp_projection_dim = min(lp_feature_dim, feature_matrix.shape[0] - 1, feature_matrix.shape[1])
            if lp_projection_dim > 0:
                lp_feature_embedding = normalize_rows(low_rank_projection(feature_matrix, lp_projection_dim))
        lp_cache = {
            "test_pos": test_pos,
            "test_neg": test_neg,
            "train_fused": train_fused,
            "train_predicted": train_predicted,
            "feature_embedding": lp_feature_embedding,
        }
        lp_runner = LouvainNERunner(REPO_ROOT, data.num_nodes, lp_embedding_dim)
        ...
        if lp_cache is not None:
            try:
                train_embeddings = build_link_prediction_embeddings(
                    runner=lp_runner,
                    fused_edges=lp_cache["train_fused"],
                    predicted_edges=lp_cache["train_predicted"],
                    seed=seed + 10000,
                    feature_matrix=None,
                    feature_dim=0,
                ...
                if lp_cache["feature_embedding"] is not None:
                    train_embeddings = torch.cat([train_embeddings, lp_cache["feature_embedding"]], dim=1)
                lp_metrics = compute_link_prediction_metrics(
                    train_embeddings,
                    lp_cache["test_pos"],
                    lp_cache["test_neg"],
                )
...
def benchmark_ogb_dataset(
    ...
    improved_predictions: Dict[Tuple[int, int], float] = {}
    projection_dim = 0
    ...
    if use_attributes and bundle.num_features > 0:
        ...
        if bundle.num_nodes <= EXACT_TOPK_MAX_NODES:
            ...
            attribute_mode = "global_topk"
        else:
            improved_edges, improved_predictions = build_structure_reweighted_edges(
                data.edge_index,
                graph_features,
                min_similarity=IMPROVED_MIN_SIMILARITY,
                overlap_scale=1.0,
            )
            attribute_mode = "structure_reweight"
    else:
    ...
    payload = {
        ...
        "graph_stats": {
            ...
            "improved_predicted_edges": len(improved_predictions) if use_attributes else 0,
