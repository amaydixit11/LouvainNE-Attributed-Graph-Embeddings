#!/usr/bin/env python3
"""
Extended benchmark script with link prediction evaluation.
Benchmarks LouvainNE methods on node classification AND link prediction.
"""
...
def evaluate_units_with_link_pred(
    name: str,
...
def benchmark_dataset_with_link_pred(dataset_name: str, embedding_dim: int) -> Dict[str, object]:
    """Benchmark dataset with both node classification and link prediction."""
    ...
    baseline = evaluate_units_with_link_pred(
    ...
    improved = evaluate_units_with_link_pred(
...
def write_summary_with_link_pred(
    summary_path: Path, 
    markdown_path: Path, 
    plot_path: Path, 
    dataset_payloads: Sequence[Dict[str, object]]
) -> None:
    ...
    lines.append("")
    lines.append("## Link Prediction Results")
    lines.append("")
    lines.append("| Dataset | Baseline Link AUC | Improved Link AUC | Baseline Link AP | Improved Link AP |")
    lines.append("|---|---:|---:|---:|---:|")
    for payload in dataset_payloads:
        base = payload["results"]["baseline"]
        improved = payload["results"]["improved"]
        lines.append(
            f"| {payload['dataset']} | "
            f"{base['link_auc_mean']:.4f} ± {base['link_auc_std']:.4f} | "
            f"{improved['link_auc_mean']:.4f} ± {improved['link_auc_std']:.4f} | "
            f"{base['link_ap_mean']:.4f} ± {base['link_ap_std']:.4f} | "
    ...
    baseline_link_auc = [payload["results"]["baseline"]["link_auc_mean"] for payload in dataset_payloads]
    improved_link_auc = [payload["results"]["improved"]["link_auc_mean"] for payload in dataset_payloads]
    ...
    axes[0, 0].bar([i - width / 2 for i in x], baseline_micro, width, label="Baseline", color="#2A6F97")
    axes[0, 0].bar([i + width / 2 for i in x], improved_micro, width, label="Improved", color="#E07A5F")
    axes[0, 0].set_xticks(list(x), datasets)
    axes[0, 0].set_ylim(0.0, 1.0)
    axes[0, 0].set_ylabel("Micro-F1")
    axes[0, 0].set_title("Node Classification Accuracy")
    axes[0, 0].legend()
    axes[0, 0].grid(axis="y", linestyle="--", alpha=0.35)

    axes[0, 1].bar([i - width / 2 for i in x], baseline_link_auc, width, label="Baseline", color="#6C9A8B")
    axes[0, 1].bar([i + width / 2 for i in x], improved_link_auc, width, label="Improved", color="#C8553D")
    axes[0, 1].set_xticks(list(x), datasets)
    axes[0, 1].set_ylim(0.0, 1.0)
    axes[0, 1].set_ylabel("Link AUC")
    axes[0, 1].set_title("Link Prediction AUC")
    axes[0, 1].legend()
    axes[0, 1].grid(axis="y", linestyle="--", alpha=0.35)

    axes[0, 2].bar([i - width / 2 for i in x], baseline_time, width, label="Baseline", color="#7D4E9E")
    axes[0, 2].bar([i + width / 2 for i in x], improved_time, width, label="Improved", color="#D1495B")
    axes[0, 2].set_xticks(list(x), datasets)
    axes[0, 2].set_ylabel("Seconds")
    axes[0, 2].set_title("Per-Seed Evaluation Time")
    axes[0, 2].legend()
    axes[0, 2].grid(axis="y", linestyle="--", alpha=0.35)
    
    for idx, value in enumerate(baseline_micro):
        axes[0, 0].text(idx - width / 2, value + 0.02, f"{value:.3f}", ha="center", va="bottom", fontsize=8)
    for idx, value in enumerate(improved_micro):
        axes[0, 0].text(idx + width / 2, value + 0.02, f"{value:.3f}", ha="center", va="bottom", fontsize=8)
    for idx, value in enumerate(baseline_link_auc):
        axes[0, 1].text(idx - width / 2, value + 0.02, f"{value:.3f}", ha="center", va="bottom", fontsize=8)
    for idx, value in enumerate(improved_link_auc):
        axes[0, 1].text(idx + width / 2, value + 0.02, f"{value:.3f}", ha="center", va="bottom", fontsize=8)

    fig.suptitle("LouvainNE Benchmark: Node Classification + Link Prediction", fontsize=14)
    fig.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Benchmark with node classification and link prediction.")
    ...
    for dataset_name in args.datasets:
        ...
        payload = benchmark_dataset_with_link_pred(dataset_name, args.embedding_dim)
        dataset_payloads.append(payload)
        base = payload["results"]["baseline"]
        improved = payload["results"]["improved"]
        print(
            payload["dataset"],
            f"node: baseline_micro={base['test_micro_f1_mean']:.4f}, improved_micro={improved['test_micro_f1_mean']:.4f}",
            f"link: baseline_auc={base['link_auc_mean']:.4f}, improved_auc={improved['link_auc_mean']:.4f}",
            f"time: baseline={base['per_seed_eval_time_seconds_mean']:.2f}s, improved={improved['per_seed_eval_time_seconds_mean']:.2f}s",
            flush=True,
        )
    ...
    write_summary_with_link_pred(summary_json, summary_md, summary_plot, dataset_payloads)
